...
// ─── Valuation Alert ──────────────────────────────────────────────────────────
// Computes the implied PEG at the current market price and fires a warning
// when the math is getting stretched (PEG > 2.0) or dangerously stretched (> 2.5).
// This is a secondary, valuation-only signal — independent of the story check.
function ValuationAlert({ result }) {
  const { currentPrice, eps, revenueGrowthPct, pegRatio, category } = result;

  // Implied PEG = (Price / EPS) / growth rate
  // We use the live price to re-derive the current implied PEG so it updates
  // dynamically if the user is looking at a price that has moved.
  const impliedPE  = eps > 0 ? currentPrice / eps : null;
  const impliedPEG = impliedPE && revenueGrowthPct > 0
    ? impliedPE / revenueGrowthPct
    : pegRatio; // fall back to stored PEG

  // Speculative / Turnaround companies don't have reliable PEG math — skip.
  if (category === "SPEC" || !impliedPEG || impliedPEG <= 0) return null;

  // Thresholds Lynch discussed
  const isStretched = impliedPEG >= 2.0 && impliedPEG < 2.5;
  const isDangerous = impliedPEG >= 2.5;

  if (!isStretched && !isDangerous) return null;

  const color  = isDangerous ? "#f87171" : "#fbbf24";
  const bg     = isDangerous ? "#1a080840" : "#1a120040";
  const border = isDangerous ? "#7f1d1d"   : "#78350f";
  const label  = isDangerous
    ? "⚠ VALUATION STRETCHED — MATH NO LONGER WORKS"
    : "◈ VALUATION ALERT — PEG GETTING EXPENSIVE";
  const msg = isDangerous
    ? `The implied PEG at today's price is ${impliedPEG.toFixed(2)} — well above Lynch's 2.5 ceiling. At this level you're pricing in perfection and leaving yourself no margin for error. Lynch's rule: a great company at a terrible price is still a terrible investment. This is a valuation-only flag — if the story is intact, consider trimming rather than selling outright.`
    : `The implied PEG at today's price is ${impliedPEG.toFixed(2)}. Lynch loved buying below 1.0 and got uncomfortable above 2.0. The business may still be great, but the math is getting stretched. Watch for any story-check failures — a weakening story at a stretched valuation is when Lynch trimmed hardest.`;
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 10, flexWrap: "wrap" }}>
        <div style={{ fontFamily: "monospace" }}>
          <span style={{ fontSize: 10, color: "#4b5563", letterSpacing: 1, textTransform: "uppercase" }}>Implied PEG  </span>
          <span style={{ fontSize: 22, fontWeight: 900, color }}>{impliedPEG.toFixed(2)}</span>
        </div>
        {/* Mini threshold bar */}
        <div style={{ flex: 1, minWidth: 160 }}>
          <div style={{ position: "relative", background: "#1a1a1a", borderRadius: 3, height: 6, overflow: "hidden" }}>
            {/* colour zones */}
            <div style={{ position: "absolute", left: 0, width: "33.3%", height: "100%", background: "#00ff8760" }} />
            <div style={{ position: "absolute", left: "33.3%", width: "33.3%", height: "100%", background: "#fbbf2460" }} />
            <div style={{ position: "absolute", left: "66.6%", width: "33.4%", height: "100%", background: "#f8717160" }} />
            {/* needle */}
            <div style={{
              position: "absolute",
              left: `${Math.min((impliedPEG / 3) * 100, 100)}%`,
              top: -2, width: 3, height: 10,
              background: color,
              borderRadius: 2,
          ...
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 9, color: "#333", marginTop: 3 }}>
            <span>0 · Cheap</span><span>1.0</span><span>2.0</span><span>3.0 · Danger</span>
          </div>
        </div>
      </div>

      <div style={{ fontSize: 12, lineHeight: 1.75, fontStyle: "italic", color: isDangerous ? "#fecaca" : "#fef3c7" }}>
        "{msg}"
      </div>
    </div>
  );
...
function PositionTracker({ result }) {
  ...
  return (
    <div style={{background:"#0a0f0a",border:"1px solid #00ff8718",borderLeft:"4px solid #00ff87",borderRadius:4,padding:20}}>
      ...
      {/* ── Valuation Alert (always visible, no cost basis required) ── */}
      <ValuationAlert result={result} />
