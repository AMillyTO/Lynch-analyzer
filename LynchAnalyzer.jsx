...
const firebaseConfig = {
  apiKey: "AIzaSyCJo3Yny05P4EN1z3qhqRpiVsBgQl7hxJo",
  authDomain: "lynchanalyzer.firebaseapp.com",
  projectId: "lynchanalyzer",
  storageBucket: "lynchanalyzer.firebasestorage.app",
  messagingSenderId: "382133638482",
  appId: "1:382133638482:web:a4af13c62c9cb8187b368b",
  measurementId: "G-68EYPC9MXQ"
};
...
function PositionTracker({ result }) {
  ...
  const docId = "position_" + result.ticker;

  // Load from Firestore on ticker change
  useEffect(() => {
    setLoading(true);
    setCostBasis("");
    setShares("");
    setChecks({});
    getDoc(doc(db, "positions", docId)).then(snap => {
      if (snap.exists()) {
        const d = snap.data();
        if (d.cost) setCostBasis(d.cost);
  ...
  // Save to Firestore
  async function save(cost, shares, checks) {
    try {
      await setDoc(doc(db, "positions", docId), { cost, shares, checks }, { merge: true });
      setSaved(true);
  ...
  if (loading) return (
    <div style={{background:"#0a0f0a",border:"1px solid #00ff8718",borderLeft:"4px solid #00ff87",borderRadius:4,padding:20}}>
      <div style={{fontSize:10,color:"#00ff87",letterSpacing:2}}>LOADING POSITION...</div>
    </div>
  );

  return (
    <div style={{background:"#0a0f0a",border:"1px solid #00ff8718",borderLeft:"4px solid #00ff87",borderRadius:4,padding:20}}>
      <div style={{fontSize:9,letterSpacing:3,color:"#00ff87",textTransform:"uppercase",marginBottom:16,display:"flex",alignItems:"center",gap:8}}>
        📊 My Position in {result.ticker}
        {saved && <span style={{color:"#00ff87",fontSize:9,letterSpacing:1}}>✓ saved</span>}
      </div>
